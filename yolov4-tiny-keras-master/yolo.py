...
class YOLO(object):
    ...
    def detect_image(self, image):
        ...
        print(end - start)
        image_info = [image, out_boxes, out_classes, out_scores]
        return image_info
        # return image
...
if __name__ == '__main__':
    yolo=YOLO()
    dirname="D:/dianzishangwu2020/mAP-master/input/images-optional/" ##该目录为测试照片的存储路径，每次测试照片的数量可以自己设定
    path=os.path.join(dirname)
    pic_list=os.listdir(path)
    count=0
    for filename in pic_list:
        tmp_file=pic_list[count]
        new_f=open("../mAP-master/input/detection-results/"+tmp_file.replace(".jpg", ".txt"), "a")  #预测坐标生成txt文件保存的路径
